...
class UserController extends BaseController
{
    ...
    */
    ...
    {
        try {
            $profilename = Input::get('username');
            
            if (Input::hasFile('fileProfile')){
                $fileProfile = Input::file('fileProfile');
                $destinationPath = 'uploads/';
                $extension = $fileProfile->getClientOriginalExtension();
                $fileprofilename =  $profilename.'.'.$extension;
                $upload_success = $fileProfile->move($destinationPath);
            } else {
                $fileprofilename = "";
            }
    ...
    */
    public function putShow($userId)
    {
        try {
            ...
            if (Input::hasFile('fileProfile')){
                ...
                $fileprofilename = Input::get('image');;
            }
            ...
            if($user->save()) {
                ...
                if(isset($suspended) && Sentry::getUser()->getId() !== $user->getId()) {
                    $this->_suspendUser($userId, $suspended);
                }
    ...
    protected function _suspendUser($userId, $value)
    {
        $throttle = Sentry::findThrottlerByUserId($userId);
        if($value === 'no' && $throttle->isSuspended() === true) {
            $throttle->unsuspend();
        } elseif($value === 'yes' && $throttle->isSuspended() === false) {
            $throttle->suspend();
        }
    }
}
